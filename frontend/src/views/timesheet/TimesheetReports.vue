<template>
  <div>
    <CRow>
      <CCol :md="12">
        <CCard class="mb-4">
          <CCardHeader>Relatórios de Timesheet</CCardHeader>
          <CCardBody>
            <CTable align="middle" class="mb-0 border" hover responsive>
              <CTableHead color="light">
                <CTableRow>
                  <CTableHeaderCell>ID / Criado por</CTableHeaderCell>
              ...
              <CTableBody>
                <CTableRow v-for="item in tableExample" :key="item.id">
                  <CTableDataCell>
                    <div>
                      <router-link :to="{ name: 'TimesheetReport'}">
                        <h6>{{ item.id }}</h6>
                      </router-link>
                    </div>
                  ...
                  <CTableDataCell>
                    <div>
                      <CBadge color="warning">{{ item.month }}</CBadge>
                    </div>
                    <div>
                      {{ item.period }}
                    </div>
