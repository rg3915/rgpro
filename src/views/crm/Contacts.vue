<template>
  <div>
            ...
            <CTable align="middle" class="mb-0 border" hover responsive>
              ...
              <CTableBody>
                <CTableRow v-for="item in tableExample" :key="item.name">
                  ...
                  <CTableDataCell>
                    <div>
                      <router-link :to="{ name: 'Contato'}">{{ item.user.name }}</router-link>
                    </div>
    ...
    <CModal size="xl" :visible="employeeModal" @close="closeEmployeeModal()">
      <CModalHeader>
        <CModalTitle v-if="is_edit">Editar Contato</CModalTitle>
        <CModalTitle v-else>Adicionar Contato</CModalTitle>
      </CModalHeader>
      <CModalBody>
        <CForm>
          <CRow>
...
export default {
  ...
  setup() {
    ...
    const progressGroupExample3 = [
      ...
      {
        id: 3,
        avatar: { src: avatar5, status: 'warning' },
        user: {
          name: 'Elliana Eakins',
